...
{
    protected readonly InvestProDbContext Db;
    protected LedgerServiceBase(InvestProDbContext db) => Db = db;

    protected abstract DbSet<T> Set(InvestProDbContext db);
    protected abstract LedgerKind Kind { get; }

    public virtual Task<List<T>> GetByInvestmentAsync(Guid investmentId, CancellationToken ct = default)
        => Set(Db)
            .Include(x => x.Partner)
    ...
    protected async Task<(bool ok, string? error)> EnsureInvestmentEditableAsync(Guid investmentId, CancellationToken ct)
    {
    ...
    {
        var (ok, err) = await EnsureInvestmentEditableAsync(investmentId, ct);
    ...
    public virtual async Task<(bool ok, string? error)> UpdateAsync(Guid id, T input, CancellationToken ct = default)
    {
        ...
        var (ok, err) = await EnsureInvestmentEditableAsync(row.InvestmentId, ct);
